...
#
# Filename:     subtitle_downloader.py
# Version:      1.0.0
# Description:  download the subtitle of the direct movie.
# Author:       Luke Lee
# History:
#       2015-10-08 | First created.
################################################################################
...
def sub_downloader(file_path):
    ...
    try:
        ...
        if not os.path.exists(root + ".srt"):
            ...
            url = DB_LINK + "?action=download&hash=" + get_hash(file_path) + "&language=en"
            if PY_VERSION == 3:
